...
class GeneratorPublishCommand extends PublishBaseCommand
{
    ...
    public function handle()
    {
        ...
        $repositoryPattern = config('deto.laravel_generator.options.repository_pattern', true);
    ...
    {
        $apiVersion = config('deto.laravel_generator.api_version', 'v1');
        $apiPrefix = config('deto.laravel_generator.api_prefix', 'api');
    ...
    {
        $testsPath = config('deto.laravel_generator.path.tests', base_path('tests/'));
        $testsNameSpace = config('deto.laravel_generator.namespace.tests', 'Tests');
        $createdAtField = config('deto.laravel_generator.timestamps.created_at', 'created_at');
        $updatedAtField = config('deto.laravel_generator.timestamps.updated_at', 'updated_at');

        $templateData = get_template('test.api_test_trait', 'deto_generator');
        ...
        $testAPIsPath = config('deto.laravel_generator.path.api_test', base_path('tests/APIs/'));
        if (!file_exists($testAPIsPath)) {
            FileUtil::createDirectoryIfNotExist($testAPIsPath);
            $this->info('APIs Tests directory created');
        }

        $testRepositoriesPath = config('deto.laravel_generator.path.repository_test', base_path('tests/Repositories/'));
    ...
    {
        $templateData = get_template('app_base_controller', 'deto_generator');
    ...
    {
        $templateData = get_template('base_repository', 'deto_generator');
